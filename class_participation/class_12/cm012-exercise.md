...
ggplot(h_gap_dropped, aes(x = year, y = lifeExp)) +
  geom_point() +
  labs(color = "country")
```
...
gapminder %>%
  ggplot() +
  geom_bar(aes(fct_relevel(continent, levels = c("Africa","Oceania", "Europe",  "Americas", "Asia" )))) +
  coord_flip()+
  theme_bw()+
  ylab("Number of entries") + xlab("continent")
```
...
```
## Warning: Unknown levels in `f`: oceania
...
```
## Error in ylab("Number of entries") + xlab("continent"): non-numeric argument to binary operator
```
